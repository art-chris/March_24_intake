...
# create aan input where user can show more data samples
st.sidebar.subheader('Show more Data')
if st.sidebar.checkbox('Show more Data'):
    #specify the number of rows to have
    st.sidebar.subheader('Showing more Data')
    num_rows = st.sidebar.number_input('Number of  rows:', 1, 100)
...
with col1:
    #visual number one(Average mpg per origin)
    st.header('Average mpg per origin')
...
# Machine learning
#print aand drop any missing values
...
st.sidebar.write('Input your cr features to make a prediction')
